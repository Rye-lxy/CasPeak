...
Finally, `plot` subcommand can make a dotplot, a.k.a. Oxford Grid, for each peak according to the result MAF file:
```
caspeak plot --maf peak/validate.maf
```
All the plots are stored in *fig* dir by default.
## Subcommands
### align
...
### plot
`caspeak plot` makes a dotplot, a.k.a. Oxford Grid, for each peak. It originates from [last-dotplot](https://gitlab.com/mcfrith/last/-/blob/main/doc/last-dotplot.rst) and inherits all options except the input.
| Option | Description |
| --- | --- |
| `--maf` <FILE\> | The result MAF file used for plotting, usually generated by `caspeak valid` or `caspeak exec`. |
...
| other options | See [last-dotplot](https://gitlab.com/mcfrith/last/-/blob/main/doc/last-dotplot.rst).|